...
One option for the first exercise is to use POX.  POX is a Python-based SDN controller platform geared towards research and education.  For more details on POX, see [https://openflow.stanford.edu/display/ONL/POX+Wiki/ the POX Wiki].
...
Here, you'll benchmark the provided of_tutorial hub. Vim is a good choice, it comes already downloaded with terminal. It has some funky commands to edit text so http://vim.rtorr.com/[this cheat sheet] can be of use. To use vim, make sure you are in the correct directory (pox/pox/misc) and enter: 
...
* [http://people.valinux.co.jp/~yamahata/ryu/openflow-tutorial/OpenFlow_Tutorial_Ryu-ovf.zip Virtual Machine Image(zipped OVF format, 64-bit, Mininet 2.0)].
